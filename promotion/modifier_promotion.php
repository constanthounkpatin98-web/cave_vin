...
if(isset($_POST["libelle"]))
{
    ...
    $valeur_remise = trim($_POST["valeur_remise"]);
    ...
    {
        header("Location: modifier_promotion.php?id_promotion=".$id_promotion."&erreur=1");
        exit();
    }

    if(strtotime($date_fin) < strtotime($date_debut))
    {
        header("Location: modifier_promotion.php?id_promotion=".$id_promotion."&erreur=dates");
        exit();
    }
    ...
    SET libelle = ?, description = ?, type_remise = ?, valeur_remise = ?, date_debut = ?, date_fin = ?, statut = ?

    WHERE id_promotion = ?

    ");

    $requete->execute([$libelle, $description, $type_remise, $valeur_remise, $date_debut, $date_fin, $statut, $id_promotion]);

    header("Location: liste_promotion.php?modifier=ok");
    exit();
}
...
$requete = $connexion->prepare("SELECT * FROM promotion WHERE id_promotion = ?");
...
/* ============================================
   STYLES POUR LA PAGE DE MODIFICATION DE PROMOTION
   ============================================ */

.modifier-promo-container {
    max-width: 900px;
    margin: 0 auto;
}
...
.modifier-promo-card .card-header {
    background: #ffc107;
    color: #000;
    padding: 1rem 1.5rem;
...
.modifier-promo-card .card-header h3 {
    font-weight: 600;
    font-size: 1.25rem;
    margin: 0;
}

.modifier-promo-card .card-body {
    padding: 1.5rem;
    background: white;
}

.modifier-promo-card .form-label {
    font-weight: 500;
    font-size: 0.875rem;
    margin-bottom: 0.25rem;
}

.modifier-promo-card .form-control,
.modifier-promo-card .form-select {
...
.modifier-promo-card .form-control:focus,
.modifier-promo-card .form-select:focus {
    border-color: #0d6efd;
...
<aside class="barre-laterale" id="sidebar">
    ...
    <nav class="nav-laterale">
        ...
        <a href="../promotion/liste_promotion.php" class="lien-nav actif"><i class="bi bi-percent"></i> Promotions</a>
...
<div class="contenu-principal">
    ...
    <div class="barre-superieure">
        ...
        <a href="liste_promotion.php" class="bouton-retour"><i class="bi bi-arrow-left"></i> Retour</a>
    ...
    <div class="zone-corps">
        ...
        <div class="modifier-promo-container">

            <!-- ============================================
                 ALERTES
                 ============================================ -->
            <?php if(isset($_GET['erreur']) && $_GET['erreur'] == 'dates'): ?>
            <div class="alert alert-danger alert-custom alert-dismissible fade show" role="alert">
                <i class="bi bi-exclamation-triangle-fill me-2"></i> La date de fin doit être postérieure à la date de début.
            ...
            <!-- ============================================
                 CARTE PRINCIPALE
                 ============================================ -->
            <div class="card modifier-promo-card">

                <!-- Bandeau jaune -->
                <div class="card-header">
                    <h3><i class="bi bi-pencil-square me-2"></i>Modifier la promotion</h3>
                </div>

                <!-- Corps avec le formulaire -->
                <div class="card-body">

                    <form action="modifier_promotion.php" method="POST">

                        <input type="hidden" name="id_promotion" value="<?php echo $promotion["id_promotion"]; ?>">

                        <div class="mb-3">
                            <label class="form-label">Libellé</label>
                            <input type="text" name="libelle" class="form-control" value="<?php echo htmlspecialchars($promotion["libelle"]); ?>" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Description</label>
                            <textarea name="description" class="form-control" rows="3"><?php echo htmlspecialchars($promotion["description"]); ?></textarea>
                        </div>

                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Type de remise</label>
                                <select class="form-select" name="type_remise" required>
                                    <option value="Pourcentage" <?php echo $promotion["type_remise"] == "Pourcentage" ? "selected" : ""; ?>>Pourcentage</option>
                                    <option value="Montant fixe" <?php echo $promotion["type_remise"] == "Montant fixe" ? "selected" : ""; ?>>Montant fixe</option>
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Valeur de la remise</label>
                                <input type="number" step="0.01" name="valeur_remise" class="form-control" value="<?php echo htmlspecialchars($promotion["valeur_remise"]); ?>" required>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="date_debut" class="form-control" value="<?php echo htmlspecialchars($promotion["date_debut"]); ?>" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Date de fin</label>
                                <input type="date" name="date_fin" class="form-control" value="<?php echo htmlspecialchars($promotion["date_fin"]); ?>" required>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="statut" required>
                                <option value="Active" <?php echo $promotion["statut"] == "Active" ? "selected" : ""; ?>>Active</option>
                                <option value="Inactive" <?php echo $promotion["statut"] == "Inactive" ? "selected" : ""; ?>>Inactive</option>
                                <option value="Expirée" <?php echo $promotion["statut"] == "Expirée" ? "selected" : ""; ?>>Expirée</option>
                            </select>
                        </div>

                        <div class="d-flex justify-content-between">
                            <a href="liste_promotion.php" class="btn btn-annuler">Annuler</a>
