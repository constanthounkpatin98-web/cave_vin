...
if(isset($_POST["titre"]))
{
    ...
    $id_client = $_POST["id_client"]; // "tous" ou un id précis

    if(empty($titre) || empty($message) || empty($id_client))
    {
        header("Location: envoyer_notification.php?erreur=1");
        exit();
    }

    if($id_client == "tous")
    {
        $requete_clients = $connexion->query("SELECT id_client FROM client WHERE statut = 'Actif'");

        $requete_insertion = $connexion->prepare("

        INSERT INTO notification (titre, message, statut, id_client)

        VALUES (?, ?, 'Non lue', ?)

        ");

        while($c = $requete_clients->fetch())
        {
            $requete_insertion->execute([$titre, $message, $c["id_client"]]);
        }
    }
    else
    {
        ...
        $requete_insertion->execute([$titre, $message, $id_client]);
    }

    header("Location: liste_notification.php?envoi=ok");
    exit();
}
...
$requete_clients = $connexion->query("SELECT id_client, nom, prenom FROM client WHERE statut = 'Actif' ORDER BY nom ASC");
...
/* ============================================
   STYLES POUR LA PAGE D'ENVOI DE NOTIFICATION
   ============================================ */

.envoyer-notif-container {
    max-width: 800px;
    margin: 0 auto;
}
...
.envoyer-notif-card .card-header {
    background: #0d6efd;
    color: white;
    padding: 1rem 1.5rem;
...
.envoyer-notif-card .card-header h3 {
    font-weight: 600;
    font-size: 1.25rem;
    margin: 0;
}

.envoyer-notif-card .card-body {
    padding: 1.5rem;
    background: white;
}

.envoyer-notif-card .form-label {
    font-weight: 500;
    font-size: 0.875rem;
    margin-bottom: 0.25rem;
}

.envoyer-notif-card .form-control,
.envoyer-notif-card .form-select {
...
.envoyer-notif-card .form-control:focus,
.envoyer-notif-card .form-select:focus {
    border-color: #0d6efd;
...
<aside class="barre-laterale" id="sidebar">
    ...
    <nav class="nav-laterale">
        ...
        <a href="../notification/liste_notification.php" class="lien-nav actif"><i class="bi bi-bell"></i> Notifications</a>
...
<div class="contenu-principal">
    ...
    <div class="barre-superieure">
        ...
        <a href="liste_notification.php" class="bouton-retour"><i class="bi bi-arrow-left"></i> Retour</a>
    ...
    <div class="zone-corps">
        ...
        <div class="envoyer-notif-container">
            ...
            <!-- ============================================
                 CARTE PRINCIPALE
                 ============================================ -->
            <div class="card envoyer-notif-card">

                <!-- Bandeau bleu -->
                <div class="card-header">
                    <h3><i class="bi bi-send me-2"></i>Envoyer une notification</h3>
                </div>

                <!-- Corps avec le formulaire -->
                <div class="card-body">

                    <form action="envoyer_notification.php" method="POST">

                        <div class="mb-3">
                            <label class="form-label">Destinataire</label>
                            <select class="form-select" name="id_client" required>
                                <option value="">.....Choisir.....</option>
                                <option value="tous">Tous les clients actifs</option>
                                <?php while($c = $requete_clients->fetch()): ?>
                                    <option value="<?php echo $c["id_client"]; ?>"><?php echo htmlspecialchars($c["nom"] . " " . $c["prenom"]); ?></option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            <a href="liste_notification.php" class="btn btn-annuler">Annuler</a>
                            <button type="submit" class="btn btn-envoyer"><i class="bi bi-send me-1"></i> Envoyer</button>
                        </div>
