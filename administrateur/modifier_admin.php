...
$id_admin_connecte = $_SESSION["admin_id"];
...
if(!isset($_SESSION["admin_nom"]) || $_SESSION["admin_nom"] === "")
{
    try
    {
        ...
        $req_nom_admin->execute([$id_admin_connecte]);
...
if(isset($_POST["nom"]))
{
    ...
    {
        header("Location: modifier_admin.php?id_admin=".$id_admin."&erreur=1");
        exit();
    }

    if(!empty($nouveau_mdp))
    {
        $mot_de_passe_hache = password_hash($nouveau_mdp, PASSWORD_DEFAULT);

        $requete = $connexion->prepare("

        UPDATE administrateur

        SET nom = ?, prenom = ?, email = ?, role = ?, statut = ?, mot_de_passe = ?

        WHERE id_admin = ?

        ");

        $requete->execute([$nom, $prenom, $email, $role, $statut, $mot_de_passe_hache, $id_admin]);
    }
    else
    {
        $requete = $connexion->prepare("
        ...
        $requete->execute([$nom, $prenom, $email, $role, $statut, $id_admin]);
    }

    header("Location: liste_admin.php?modifier=ok");
    exit();
}
...
$requete = $connexion->prepare("SELECT * FROM administrateur WHERE id_admin = ?");
...
/* ============================================
   STYLES POUR LA PAGE DE MODIFICATION D'ADMINISTRATEUR
   ============================================ */

.modifier-admin-container {
    max-width: 900px;
    margin: 0 auto;
}
...
.modifier-admin-card .card-header {
    background: #ffc107;
    color: #000;
    padding: 1rem 1.5rem;
...
.modifier-admin-card .card-header h3 {
    font-weight: 600;
    font-size: 1.25rem;
    margin: 0;
}

.modifier-admin-card .card-body {
    padding: 1.5rem;
    background: white;
}

.modifier-admin-card .form-label {
    font-weight: 500;
    font-size: 0.875rem;
    margin-bottom: 0.25rem;
}

.modifier-admin-card .form-control,
.modifier-admin-card .form-select {
...
.modifier-admin-card .form-control:focus,
.modifier-admin-card .form-select:focus {
    border-color: #0d6efd;
...
<aside class="barre-laterale" id="sidebar">
    ...
    <nav class="nav-laterale">
        ...
        <a href="../administrateur/liste_admin.php" class="lien-nav actif"><i class="bi bi-person-badge"></i> Administrateurs</a>
        <a href="../rapports/rapports.php" class="lien-nav"><i class="bi bi-file-earmark-bar-graph"></i> Rapports</a>
        <a href="../parametres/parametres.php" class="lien-nav"><i class="bi bi-gear"></i> Paramètres</a>
    </nav>
...
<div class="contenu-principal">
    ...
    <div class="barre-superieure">
        ...
        <a href="liste_admin.php" class="bouton-retour"><i class="bi bi-arrow-left"></i> Retour</a>
    ...
    <div class="zone-corps">
        ...
        <div class="modifier-admin-container">
            ...
            <!-- ============================================
                 CARTE PRINCIPALE
                 ============================================ -->
            <div class="card modifier-admin-card">

                <!-- Bandeau jaune -->
                <div class="card-header">
                    <h3><i class="bi bi-pencil-square me-2"></i>Modifier l'administrateur</h3>
                </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" name="role" required>
                                    <option value="Administrateur" <?php echo $admin["role"] == "Administrateur" ? "selected" : ""; ?>>Administrateur</option>
                                    <option value="Super Administrateur" <?php echo $admin["role"] == "Super Administrateur" ? "selected" : ""; ?>>Super Administrateur</option>
                                    <option value="Gestionnaire Stock" <?php echo $admin["role"] == "Gestionnaire Stock" ? "selected" : ""; ?>>Gestionnaire Stock</option>
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Statut</label>
                                <select class="form-select" name="statut" required>
                                    <option value="Actif" <?php echo $admin["statut"] == "Actif" ? "selected" : ""; ?>>Actif</option>
                                    <option value="Inactif" <?php echo $admin["statut"] == "Inactif" ? "selected" : ""; ?>>Inactif</option>
                                </select>
                            </div>
                        </div>

                        <div class="d-flex justify-content-between">
                            <a href="liste_admin.php" class="btn btn-annuler">Annuler</a>
