...
if(isset($_POST["nom"]))
{
    ...
    {
        header("Location: ajout_admin.php?erreur=champs");
        exit();
    }

    if($mot_de_passe !== $confirmation)
    {
        header("Location: ajout_admin.php?erreur=mdp");
        exit();
    }
    ...
    if($verif->fetch())
    {
        header("Location: ajout_admin.php?erreur=email");
        exit();
    }
    ...
    $requete->execute([$nom, $prenom, $email, $mot_de_passe_hache, $role, $statut]);

    header("Location: liste_admin.php?ajout=ok");
    exit();
}
...
/* ============================================
   STYLES POUR LA PAGE D'AJOUT D'ADMINISTRATEUR
   ============================================ */

.ajout-admin-container {
    max-width: 900px;
    margin: 0 auto;
}
...
.ajout-admin-card .card-header h3 {
    font-weight: 600;
    font-size: 1.25rem;
    margin: 0;
}

.ajout-admin-card .card-body {
    padding: 1.5rem;
    background: white;
}

.ajout-admin-card .form-label {
    font-weight: 500;
    font-size: 0.875rem;
    margin-bottom: 0.25rem;
}

.ajout-admin-card .form-control,
.ajout-admin-card .form-select {
...
.ajout-admin-card .form-control:focus,
.ajout-admin-card .form-select:focus {
    border-color: #0d6efd;
...
<div class="contenu-principal">
    ...
    <div class="zone-corps">
        ...
        <div class="ajout-admin-container">
            ...
            <div class="alert alert-danger alert-custom alert-dismissible fade show" role="alert">
                <i class="bi bi-exclamation-triangle-fill me-2"></i> Les mots de passe ne correspondent pas.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Fermer"></button>
            </div>
            <?php elseif(isset($_GET['erreur']) && $_GET['erreur'] == 'email'): ?>
            <div class="alert alert-danger alert-custom alert-dismissible fade show" role="alert">
                <i class="bi bi-exclamation-triangle-fill me-2"></i> Cet email est déjà utilisé.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Fermer"></button>
            </div>
            <?php endif; ?>
            ...
            <!-- ============================================
                 CARTE PRINCIPALE
                 ============================================ -->
            <div class="card ajout-admin-card">

                <!-- Bandeau bleu -->
                <div class="card-header">
                    <h3><i class="bi bi-person-plus me-2"></i>Ajouter un nouvel administrateur</h3>
                </div>
