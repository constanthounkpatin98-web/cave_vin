...
//===============================================
// Récupération des paiements (avec infos commande et client)
//===============================================
...
try
{
    ...
    SELECT paiement.*, commande.id_commande, client.nom AS nom_client, client.prenom AS prenom_client
...
//===============================================
// Normalisation du statut (tolérant aux soucis d'encodage
// / espaces en trop provenant de la base de données)
//===============================================
...
function normaliser_statut($valeur)
{
    ...
    // Si la chaîne n'est pas de l'UTF-8 valide (souvent le cas si la
    // connexion PDO ou la table est en latin1), on tente de la reconvertir
    // pour éviter que htmlspecialchars() ne la vide silencieusement.
...
{
    // Compare en ignorant la casse et les accents, pour être tolérant
    // aux petites différences d'encodage / de saisie.
...
/* ============================================
   ...
   (IDENTIQUE À LA LISTE DES CLIENTS)
   ============================================ */
...
<aside class="barre-laterale" id="sidebar">
    ...
    <nav class="nav-laterale">
        ...
        <a href="liste_paiement.php" class="lien-nav actif"><i class="bi bi-credit-card"></i> Paiements</a>
    ...
    <div class="zone-corps">
        ...
        <div class="liste-commande-container">
            ...
            <div class="alert alert-danger alert-custom alert-dismissible fade show" role="alert">
                ...
                <strong>Les paiements n'ont pas pu être chargés.</strong>
            ...
            <div class="alert alert-info alert-custom alert-dismissible fade show" role="alert">
                <i class="bi bi-info-circle-fill me-2"></i> Le statut du paiement a été mis à jour.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Fermer"></button>
            </div>
            <?php endif; ?>
            ...
            <div class="alert alert-danger alert-custom alert-dismissible fade show" role="alert">
                <i class="bi bi-exclamation-triangle-fill me-2"></i> <?php echo htmlspecialchars($_GET["erreur"]); ?>
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Fermer"></button>
            </div>
            <?php endif; ?>

            <!-- ============================================
                 CARTE PRINCIPALE - MÊME STRUCTURE QUE LES CLIENTS
                 ============================================ -->
            <div class="card liste-commande-card">

                <!-- Bandeau bleu -->
                <div class="bandeau-commande">
                    <h3><i class="bi bi-credit-card me-2"></i>Liste des Paiements</h3>
                </div>
                ...
                <div class="card-body">
                    ...
                    <table id="tableau" class="table table-bordered table-striped table-hover">
                        ...
                        <tbody>
                            <?php foreach($paiements as $ligne): ?>
                            <tr>
                                <td><?php echo $ligne["id_paiement"]; ?></td>
                                <td>#<?php echo $ligne["id_commande"]; ?></td>
                                <td class="fw-semibold"><?php echo htmlspecialchars($ligne["nom_client"] . " " . $ligne["prenom_client"]); ?></td>
                                <td><?php echo htmlspecialchars($ligne["mode_paiement"]); ?></td>
                                <td><?php echo number_format($ligne["montant"], 0, ',', ' '); ?> FCFA</td>
                                <td><?php echo htmlspecialchars($ligne["reference_transaction"]); ?></td>
                                <td><?php echo htmlspecialchars($ligne["date_paiement"]); ?></td>
                                <td>
                                    <?php
                                    $statut_propre = normaliser_statut($ligne["statut"]);
                                    $couleur_badge = "secondary";
                                    if(statut_correspond($statut_propre, "Validé"))    $couleur_badge = "success";
                                    if(statut_correspond($statut_propre, "Échoué"))    $couleur_badge = "danger";
                                    if(statut_correspond($statut_propre, "Remboursé")) $couleur_badge = "warning";
                                    $texte_statut = $statut_propre !== '' ? $statut_propre : "Non défini";
                                    ?>
                                    <span class="badge rounded-pill text-bg-<?php echo $couleur_badge; ?>"><?php echo htmlspecialchars($texte_statut, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?></span>
                                </td>
                                <td class="text-nowrap">
                                    <div class="actions-ligne">
                                        <?php if(statut_correspond($statut_propre, "En attente")): ?>
                                        <span class="text-muted small"><i class="bi bi-clock-history"></i> En attente de confirmation client</span>
                                        <?php elseif(statut_correspond($statut_propre, "Validé")): ?>
                                        <a href="modifier_statut_paiement.php?id_paiement=<?php echo $ligne["id_paiement"]; ?>&statut=Remboursé" class="btn btn-warning btn-sm" onclick="return confirm('Confirmer le remboursement ?');">Rembourser</a>
                                        <?php else: ?>
