...
if(isset($_POST["statut"]))
{
    ...
    $statut         = $_POST["statut"];
    $mode_livraison = trim($_POST["mode_livraison"]);

    if(empty($statut))
    {
        header("Location: modifier_commande.php?id_commande=".$id_commande."&erreur=1");
        exit();
    }
    ...
    SET statut = ?, mode_livraison = ?

    WHERE id_commande = ?

    ");

    $requete->execute([$statut, $mode_livraison, $id_commande]);

    header("Location: liste_commande.php?modifier=ok");
    exit();
}
...
.bandeau-commande.bandeau-jaune{
    background:#ffc107;
    color:#000;
}
    ...
    <div class="zone-corps">
        ...
        <div class="liste-commande-container">
            ...
            <div class="card liste-commande-card">
                ...
                <div class="bandeau-commande bandeau-jaune">
                    <h3><i class="bi bi-pencil-square me-2"></i>Modifier la commande #<?php echo $commande["id_commande"]; ?></h3>
                </div>

                <div class="card-body">

                    <div class="bloc-info mb-4">
                        <p>
                            <strong>Client :</strong> <?php echo htmlspecialchars($commande["nom_client"] . " " . $commande["prenom_client"]); ?><br>
                            <strong>Montant total :</strong> <?php echo number_format($commande["montant_total"], 0, ',', ' '); ?> FCFA
                        </p>
                    </div>
                    ...
                    <form action="modifier_commande.php" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="mode_livraison" class="form-control" value="<?php echo htmlspecialchars($commande["mode_livraison"]); ?>">
                        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            <a href="liste_commande.php" class="btn btn-secondary">Annuler</a>
