...
if(isset($_POST["libelle"]))
{
    ...
    if(empty($libelle) || empty($statut))
    {
        header("Location: modifier_categorie.php?id_categorie=".$id_categorie."&erreur=1");
        exit();
    }
    ...
    SET libelle = ?, description = ?, statut = ?

    WHERE id_categorie = ?

    ");

    $requete->execute([$libelle, $description, $statut, $id_categorie]);

    header("Location: liste_categorie.php?modifier=ok");
    exit();
}
...
$requete = $connexion->prepare("SELECT * FROM categorie WHERE id_categorie = ?");
...
/* ============================================
   STYLES POUR LA PAGE DE MODIFICATION DE CATÉGORIE
   ============================================ */

.modifier-categorie-container {
    max-width: 800px;
    margin: 0 auto;
}

.modifier-categorie-card {
    border: none;
    border-radius: 8px;
...
.modifier-categorie-card .card-header {
    background: #ffc107;
    color: #000;
    padding: 1rem 1.5rem;
...
.modifier-categorie-card .card-header h3 {
    font-weight: 600;
    font-size: 1.25rem;
    margin: 0;
}

.modifier-categorie-card .card-body {
    padding: 1.5rem;
    background: white;
}

.modifier-categorie-card .form-label {
    font-weight: 500;
    font-size: 0.875rem;
    margin-bottom: 0.25rem;
}

.modifier-categorie-card .form-control,
.modifier-categorie-card .form-select {
...
.modifier-categorie-card .form-control:focus,
.modifier-categorie-card .form-select:focus {
    border-color: #0d6efd;
...
<aside class="barre-laterale" id="sidebar">
    ...
    <nav class="nav-laterale">
        ...
        <a href="../categorie/liste_categorie.php" class="lien-nav actif"><i class="bi bi-tags"></i> Catégories</a>
...
<div class="contenu-principal">
    ...
    <div class="barre-superieure">
        ...
        <a href="liste_categorie.php" class="bouton-retour"><i class="bi bi-arrow-left"></i> Retour</a>
    ...
    <div class="zone-corps">
        ...
        <div class="modifier-categorie-container">
            ...
            <!-- ============================================
                 CARTE PRINCIPALE
                 ============================================ -->
            <div class="card modifier-categorie-card">

                <!-- Bandeau jaune -->
                <div class="card-header">
                    <h3><i class="bi bi-pencil-square me-2"></i>Modifier la catégorie</h3>
                </div>
                ...
                <div class="card-body">
                    ...
                    <form action="modifier_categorie.php" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="libelle" class="form-control" value="<?php echo htmlspecialchars($categorie["libelle"]); ?>" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Description</label>
                            <textarea name="description" class="form-control" rows="3"><?php echo htmlspecialchars($categorie["description"]); ?></textarea>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Statut</label>
                            <select class="form-select" name="statut" required>
                                <option value="Actif" <?php echo $categorie["statut"] == "Actif" ? "selected" : ""; ?>>Actif</option>
                                <option value="Inactif" <?php echo $categorie["statut"] == "Inactif" ? "selected" : ""; ?>>Inactif</option>
                            </select>
                        </div>

                        <div class="d-flex justify-content-between">
                            <a href="liste_categorie.php" class="btn btn-annuler">Annuler</a>
