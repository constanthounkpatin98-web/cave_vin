...
if(isset($_POST["id_vin"]))
{
    ...
    if(empty($id_vin) || empty($type_mouvement) || $quantite < 1)
    {
        header("Location: ajout_mouvement.php?erreur=1");
        exit();
    }
    ...
    try
    {
        ...
        $requete_vin = $connexion->prepare("SELECT quantite_stock FROM vin WHERE id_vin = ? FOR UPDATE");
        $requete_vin->execute([$id_vin]);
        $vin = $requete_vin->fetch();

        if(!$vin)
        {
            throw new Exception("Vin introuvable.");
        }

        if($type_mouvement == "Sortie" && $vin["quantite_stock"] < $quantite)
        {
        ...
        {
            $nouveau_stock = $vin["quantite_stock"] + $quantite;
        }
        else
        {
            $nouveau_stock = $vin["quantite_stock"] - $quantite;
        ...
        // Mise à jour du stock du vin
        $requete_maj = $connexion->prepare("UPDATE vin SET quantite_stock = ? WHERE id_vin = ?");
        $requete_maj->execute([$nouveau_stock, $id_vin]);
        ...
        $requete_mouvement->execute([$type_mouvement, $quantite, $nouveau_stock, $id_vin, $id_admin]);

        $connexion->commit();

        header("Location: liste_mouvement.php?ajout=ok");
        exit();
    }
    catch(Exception $e)
    {
        ...
        header("Location: ajout_mouvement.php?erreur=" . urlencode($e->getMessage()));
        exit();
    }
}
...
//===============================================
// Récupération des vins pour le select
//===============================================

$requete_vins = $connexion->query("SELECT id_vin, nom_vin, quantite_stock FROM vin ORDER BY nom_vin ASC");
...
/* ============================================
   STYLES POUR LA PAGE D'AJOUT DE MOUVEMENT
   ============================================ */

.ajout-mouvement-container {
    max-width: 800px;
    margin: 0 auto;
}

.ajout-mouvement-card {
    border: none;
    border-radius: 8px;
...
.ajout-mouvement-card .card-header {
    background: #0d6efd;
    color: white;
    padding: 1rem 1.5rem;
...
.ajout-mouvement-card .card-header h3 {
    font-weight: 600;
    font-size: 1.25rem;
    margin: 0;
}

.ajout-mouvement-card .card-body {
    padding: 1.5rem;
    background: white;
}

.ajout-mouvement-card .form-label {
    font-weight: 500;
    font-size: 0.875rem;
    margin-bottom: 0.25rem;
}

.ajout-mouvement-card .form-control,
.ajout-mouvement-card .form-select {
...
.ajout-mouvement-card .form-control:focus,
.ajout-mouvement-card .form-select:focus {
    border-color: #0d6efd;
...
<aside class="barre-laterale" id="sidebar">
    ...
    <nav class="nav-laterale">
        ...
        <a href="../stock/liste_mouvement.php" class="lien-nav actif"><i class="bi bi-box-seam"></i> Stock &amp; Mouvements</a>
...
<div class="contenu-principal">
    ...
    <div class="barre-superieure">
        ...
        <a href="liste_mouvement.php" class="bouton-retour"><i class="bi bi-arrow-left"></i> Retour</a>
    ...
    <div class="zone-corps">
        ...
        <div class="ajout-mouvement-container">
            ...
            <div class="alert alert-danger alert-custom alert-dismissible fade show" role="alert">
                <i class="bi bi-exclamation-triangle-fill me-2"></i> Veuillez remplir tous les champs correctement.
            ...
            <!-- ============================================
                 CARTE PRINCIPALE
                 ============================================ -->
            <div class="card ajout-mouvement-card">

                <!-- Bandeau bleu -->
                <div class="card-header">
                    <h3><i class="bi bi-box-seam me-2"></i>Enregistrer un mouvement de stock</h3>
                </div>

                <!-- Corps avec le formulaire -->
                <div class="card-body">

                    <form action="ajout_mouvement.php" method="POST">

                        <div class="mb-3">
                            <label class="form-label">Vin</label>
                            <select class="form-select" name="id_vin" id="id_vin" required>
                                <option value="">.....Choisir.....</option>
                                <?php while($v = $requete_vins->fetch()): ?>
                                    <option value="<?php echo $v["id_vin"]; ?>" data-stock="<?php echo $v["quantite_stock"]; ?>">
                                        <?php echo htmlspecialchars($v["nom_vin"]); ?> (stock actuel : <?php echo $v["quantite_stock"]; ?>)
                                    </option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" name="type_mouvement" required>
                                    ...
                                    <option value="Sortie">Sortie (retrait / perte / casse)</option>
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                        ...
                        <div class="d-flex justify-content-between">
                            <a href="liste_mouvement.php" class="btn btn-annuler">Annuler</a>
